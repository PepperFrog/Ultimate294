...
namespace SCP294.Classes
{
    public class SCP294Object
    {
                ...
                foreach (Player player in Player.List)
                {
                    ...
                    if (CanPlayerUse294(player))
                    {
                        if (!SCP294.Instance.PlayersNear294.Contains(player.UserId))
                        {
                            ...
                            if (SCP294.Instance.SCP294UsesLeft.Keys.Contains(scp294) &&
                                SCP294.Instance.SCP294UsesLeft[scp294] == 0)
                            {
                                player.ShowHint(
                                    "<size=300>\n</size>\n<size=35>You Approach SCP-294.</size>\n<size=30>It seems to have lost all power, rendering it unusable for now...</size>",
                                    3);
                            }
                            else
                            {
                                player.ShowHint(
                                    "<size=300>\n</size>\n<size=35>You Approach SCP-294.</size>\n<size=30>If you had a coin, you could buy a drink...</size>\n<size=20>(Hold a Coin, Open Console, Use the command '.scp294 <drink>' to dispense your drink of choice)</size>",
                                    3);
                            }
        ...
        public static void SpawnSCP294()
        {
            ...
            {
                Room SpawnRoom =
                    RoomHandler.GetRandomRoom(SCP294.Instance.Config.SpawningLocations.SpawnRooms.Keys.ToList());
                if (!SpawnRoom) continue;
                SpawnTransform relativeOffsetTransform =
                    SCP294.Instance.Config.SpawningLocations.SpawnRooms[SpawnRoom.Type].RandomItem();

                CreateSCP294(SpawnRoom.Position + (SpawnRoom.Rotation * relativeOffsetTransform.Position),
                    Quaternion.Euler(SpawnRoom.Rotation.eulerAngles + Quaternion
                        .Euler(relativeOffsetTransform.Rotation.x, relativeOffsetTransform.Rotation.y,
                            relativeOffsetTransform.Rotation.z).eulerAngles), relativeOffsetTransform.Scale);
            }
        }
        ...
        public static bool CanPlayerUse294(Player player)
        {
            ...
            {
                if (!scp294) continue;
                if (Vector3.Distance(player.Position, scp294.Position) < SCP294.Instance.Config.UseDistance)
                    return true;
            }
        ...
        /// <returns></returns>
        public static Pickup CreateDrinkPickup(Item item, Vector3 position, Quaternion rotation = default(Quaternion),
            bool spawn = true)
        {
            ItemPickupBase itemPickupBase =
                UnityEngine.Object.Instantiate(item.Base.PickupDropModel, position, rotation);
